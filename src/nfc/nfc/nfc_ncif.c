...
void nfc_ncif_cmd_timeout (void)
{
    ...
    NFC_TRACE_ERROR0 ("Recovery Start!");
    {
        ...
        */
        ...
        buffer = (UINT8 *) malloc((bufflen+1)*sizeof(UINT8));
        if(NULL == buffer)
        {
...
void nfc_ncif_storeScreenState(UINT8 state)
{
    ...
    screenState = state;
    NFC_TRACE_DEBUG1 ("nfc_ncif_storeScreenState gScreenState : %d", screenState);
}
#endif
/*******************************************************************************
**
...
void nfc_ncif_setScreenState(UINT8 state)
{
    /* store the screen state */
    screenState = state;
}
