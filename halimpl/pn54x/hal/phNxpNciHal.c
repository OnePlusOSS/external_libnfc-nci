...
// recovery --end
    ...
    NXPLOG_NCIHAL_D ("EEPROM_fw_dwnld_flag : 0x%02x SetConfigAlways flag : 0x%02x isNxppathModified : 0x%02x", fw_dwnld_flag, setConfigAlways,isNxppathModified());

    if((TRUE == fw_dwnld_flag) || (TRUE == setConfigAlways) || isNxppathModified())
    {
        config_access = TRUE;
        retlen = 0;
    ...
    if((fw_dwnld_flag == 0x01) || (TRUE == setConfigAlways) || isNxppathModified())
    {
        retlen = 0;
#if(NFC_NXP_CHIP_TYPE != PN547C2)
