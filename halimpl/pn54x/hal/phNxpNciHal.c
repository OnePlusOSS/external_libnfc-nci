...
// recovery --end
    ...
    if((TRUE == fw_dwnld_flag) || (TRUE == setConfigAlways) || (isNxpConfigModified()))
    {
        config_access = TRUE;
        retlen = 0;
        NXPLOG_NCIHAL_D ("nfc update rf config");
    ...
    if((*p_core_init_rsp_params > 0) && (*p_core_init_rsp_params < 4))
    {
        static phLibNfc_Message_t msg;
        uint16_t tmp_len = 0;
		uint8_t enable_nfcee_disc[] = {0x22, 0x00, 0x01, 0x01};
		...
		NXPLOG_NCIHAL_E("Sending NFCEE discover command as raw packet!!");
		status = phNxpNciHal_send_ext_cmd (sizeof(enable_nfcee_disc), enable_nfcee_disc);
		if (status != NFCSTATUS_SUCCESS)
		{
    		NXPLOG_NCIHAL_E("Sending NFCEE discover command as raw packet!! Failed");
    		retry_core_init_cnt++;
    		goto retry_core_init;
		}
        ...
        //p_core_init_rsp_params[255] stored the global previous screen state
        if (p_core_init_rsp_params[255] == 1) //SCREEN_STATE_OFF
        {
            set_screen_state[3] = 0x01;
        }
        else if (p_core_init_rsp_params[255] == 2) //SCREEN_STATE_LOCK,
        {
             set_screen_state[3] = 0x02;
        }
        else if (p_core_init_rsp_params[255] == 3) //SCREEN_STATE_UNLOCK,
        {
             set_screen_state[3] = 0x00;
        }
