        ...
        if (1 == gpphTmlNfc_Context->tReadInfo.bEnable)
        {
            ...
            if (NFCSTATUS_INVALID_DEVICE != (uintptr_t)gpphTmlNfc_Context->pDevHandle)
            {
                ...
                else
                {
                    ...
                    property_get("nfc.debug.core.reset", value, "false");
                    if((strcmp(value, "true")) == 0)
                    {

                        NXPLOG_TML_E("nfc.debug.core.reset true\n");
                        memcpy(gpphTmlNfc_Context->tReadInfo.pBuffer, "\x60\x00\x06\xA0\x00\x93\x13\x00\x00", 9);
                        gpphTmlNfc_Context->tReadInfo.wLength=9;
                        property_set("nfc.debug.core.reset", "false");
                    }
                    */
